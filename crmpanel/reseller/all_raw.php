...
if(!empty($del_id))
{
	$q=$db->exeQuery("select * from am_rawmaterial where id='".$del_id."' and am_status='Yes'");
	$r=$q->fetch_assoc();
	$picture='./images/rawmat/'.$r['image'];
	chmod($picture, 0644);
	unlink($picture);
	$db->exeQuery("delete from am_rawmaterial where id='".$del_id."' and am_status='Yes'");
	$error="<span class='alert alert-danger'>Data Deleted Successfully</span>";
	header('Refresh:5;url='.BASE_PATH.'All_raw/');
}
										...
										<div class="row">
											<div class="col-md-12">
												<div class="card card-box">
													<div class="card-head">
														<header>All Raw Materials</header>
														<div class="btn-group pull-right" style="padding-right:10px; padding-bottom:5px;">
															<a href="<?php echo BASE_PATH;?>Add_raw/" id="addRow" class="btn btn-primary">
																Add New Raw Materials <i class="fa fa-plus"></i>
															</a>
														</div>
													</div>
													<div class="card-body ">
														...
														<div class="table-scrollable">
															<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
																...
																<tbody>
																<?php 
																	$qr=$db->exeQuery("select * from am_rawmaterial where am_status='Yes'");
																	$i=1;
																	while($row=$qr->fetch_assoc() )
																	{
																		...
																		<td>';
																		if($row['image']!=''){
																			echo '<img src="'.BASE_PATH.'images/rawmat/'.$row['image'].'" style="height:50px;" />';
																		}
																		...
																		<td>
																			<a href="'.BASE_PATH.'Add_raw/'.$row['id'].'/" class="btn btn-primary btn-xs">
																				<i class="fa fa-pencil"></i>
																			</a>
																			<a class="btn btn-danger btn-xs" href="'.BASE_PATH.'All_raw/'.$row['id'].'/" onclick="return confirm(\'Are You sure to Delete this Record \');">
