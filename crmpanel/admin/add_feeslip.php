...
$q=$db->exeQuery("select * from mi_feeslip where id='".$edit_id."' ");
$row=$q->fetch_assoc();
									...
									<div class="row">
										...
										<div class="col-md-4 col-sm-6">
											<div class="form-group">
												...
												if($row['image']!='')
												{
													echo '<img id="upload1" src="'.WEB_PATH.'school/images/fee_slip_img/'.$row['image'].'" class="img-responsive" />';
												}else{
...
$(document).ready(function(){
	...
	$("#act-form").on("submit",function(e){
		...
		$("#preloader").show();
		$.ajax({
			url:"<?php echo BASE_PATH;?>Controller/FEESLIP/",
			type:'post',
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>All_Feeslip/';},500);
				}
				else
				{
					$("#msg").html(response.message);
