			...
			<div class="row">
				<div class="col-md-12">
					<div class="card card-box">
						<div class="card-head">
							<header>Your Daily Work Sheet</header>
						</div>
						<div class="card-body ">
						<form method="post">
							<div class="row">
							...
							<div class="table-scrollable">
								...
								if($_SESSION[SITE_NAME]['MICMP_usertype']=='Admin'){
									$qr=$db->exeQuery("select * from mi_lead_activity where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and date(plan_date)>='".date("Y-m-d",strtotime($fr_date))."' and date(plan_date)<='".date("Y-m-d",strtotime($to_date))."' and mi_status='Yes' order by plan_date desc");
								}else{
									$qr=$db->exeQuery("select * from mi_lead_activity where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and date(plan_date)>='".date("Y-m-d",strtotime($fr_date))."' and date(plan_date)<='".date("Y-m-d",strtotime($to_date))."' and plan_for='".$_SESSION[SITE_NAME]['MICMP_userid']."' and mi_status='Yes' order by plan_date desc");
								}
				...
				<form id="activity-form" autocomplete="off" enctype="multipart/form-data">
					...
					<div class="col-md-12">
						...
						<div class="form-group row">
							...
							<input size="30" class="form-control" type="date" value="<?php echo date("Y-m-d");?>" name="act_date" required  />
							</div>
							<!--<div class="col-md-8 col-sm-6 input-append date form_date" data-date-format="dd-mm-yyyy" data-date="<?php echo date("d-m-Y");?>"  data-provide="datetimepicker-inline">
								<input size="30" class="form-control" type="text" value="<?php echo date("d-m-Y");?>" name="act_date"  required style="width:85%;float:left;" />
								<span class="add-on"><i class="fa fa-remove icon-remove"></i></span>
								<span class="add-on"><i class="fa fa-calendar"></i></span>
							</div>--->
						</div>
