...
if($method=='Add')
{
	$sql=$db->exeQuery("select * from mi_industry where id='".$edit_id."' and mi_status='Yes'");
	$row=$sql->fetch_assoc();
	$industry=$row['industry'];
	$industry_id=$row['id'];
	$edit_id='';
}else{
	$sql=$db->exeQuery("select * from mi_segment where id='".$edit_id."' and mi_status='Yes'");
	$row=$sql->fetch_assoc();
	$sql1=$db->exeQuery("select * from mi_industry where id='".$row['industry']."' and mi_status='Yes'");
...
<html lang="en">
<head>
<title>ADD/EDIT Industry-Segment </title>
			...
			<div class="page-content-wrapper">
				<div class="page-content">
					<div class="row">
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50"  value="<?php echo $industry;?>" readonly/>
											<input type="hidden" name="industry[]" value="<?php echo $industry_id;?>" />
										</div>
									</div>
									<div class="col-md-10 col-sm-10">
										<div class="form-group">
											<label>Segment Name *</label>
											<input type="text" class="form-control" maxlength="50" name="segment[]" value="<?php echo $row['segment'];?>" required/>
						...
						<div class="card-box">
							<div class="card-head">
								<header>All Segment for <?php echo $industry;?> Industry </header>
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_segment where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and industry='".$industry_id."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$row['segment']."</td><td><a href='".BASE_PATH."Add_Segment/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeleteSegment";
	...
	function display()
	{
		...
		var datastr="industry=<?php echo $industry_id;?>&post_id=<?php echo $post_id;?>&pg_pmsn="+per1+"&method=ViewSegment";
		$("#displaydata").html("<i class='fa fa-spinner fa-spin'></i>");
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/INDUSTRY/',
			method:'post',
	...
	$("body").on("click","#addmore",function(e){
		e.preventDefault();
		$("#moredata").append('<div class="col-md-12 col-sm-12"><div class="form-group"><input type="text" class="form-control" maxlength="50" name="segment[]" value="" /></div></div>');
	});
