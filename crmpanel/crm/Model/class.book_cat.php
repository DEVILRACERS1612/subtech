...
class ADDBOOKCAT{
    private $conn;
    private $table_name = "mi_book_cat";
	...
	public function find_id()
	{
		...
		{
			if($this->conn->num_rows("select * from ".$this->table_name." where school_id='".$_SESSION['MISCHOOL_schoolid']."'  and ( cat_name='".$this->cat_name."') and mi_status='Yes'") )
			{
				return true;
			}else{
				return false;
			...
			if($this->conn->num_rows("select * from ".$this->table_name." where school_id='".$_SESSION['MISCHOOL_schoolid']."' and (cat_name='".$this->cat_name."') and id<>'".$this->edit_id."' and mi_status='Yes'") )
			{
				return true;
			}else{
				return false;
    ...
    public function insert(){
		...
		{
			$query = "INSERT INTO ".$this->table_name."(`id`, `rdate`, `school_id`,`sess_year`, `user_id`,  `cat_name`, `description`, `mi_status`) VALUES ('0', '".$this->rdate."','".$_SESSION['MISCHOOL_schoolid']."','".$_SESSION['sess_year']."','".$_SESSION['MISCHOOL_userid']."','".$this->cat_name."','".$this->description."','Yes')";
	...
	public function update(){
       ...
       $this->edit_id=$this->conn->filterVar($this->edit_id);
		...
		{
	   		$query = "update ".$this->table_name." set `rdate`='".$this->rdate."',`cat_name`='".$this->cat_name."',`description`='".$this->description."' where id='".$this->edit_id."' and school_id='".$_SESSION['MISCHOOL_schoolid']."'";
			if($this->conn->exeQuery($query)){
				return true;
			}else{
				return false;
	...
	public function view(){
		if($this->permission['pg_view']=='Yes' or $_SESSION['MISCHOOL_usertype']=='Admin')
		{
			...
			while($row=$qr->fetch_assoc())
			{
				$str.="<tr><td>".$i."</td><td>".$row['cat_name']."</td><td>".$row['description']."</td><td><a href='".BASE_PATH."Add_Book_Cat/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' data-per='".$this->permission."'><i class='fa fa-trash-o '></i></a></td></tr>";
				$i++;
			}
			return $str;
		}else{
