...
$sql=$db->exeQuery("select * from mi_jobs where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
					...
					<div class="row">
						<div class="col-sm-7">
							<div class="card-box">
								...
								<input type="hidden" name="method" value="Jobs" />
							...
							<div class="col-md-6 col-sm-6">
								<div class="form-group">
									...
									<?= $objcat->jobcat_list($row['cat_name']);?>
									<option value="New">New</option>
									</select>
									
									<input type="text" class="form-control" id="jcatname" name="cat_name" value="<?php echo $row['cat_name'];?>" />
							...
							<div class="col-md-12 col-sm-6">
								<div class="form-group">
									...
									<textarea class="form-control" id="summernote" name="sdes"><?php echo $row['sdes'];?></textarea>
								</div>
							</div>
							<!---<div class="col-md-12 col-sm-6">
							<div class="row">
							...
							<div class="col-md-2 col-sm-6">
								<div class="form-group">
									...
									/*if($row['image']!='')
									{
										echo '<img id="upload1" src="'.WEB_PATH.'images/news_img/'.$row['image'].'" class="img-responsive" />';
									}else{
						...
						<div class="card-body ">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select * from mi_jobs where  mi_status='Yes'");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										echo "<tr><td>".$i."</td><td>".$row['title']."</td><td><a href='".BASE_PATH."Add_Jobs/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."'><i class='fa fa-trash-o '></i></a></td></tr>";
										$i++;
									}
									?>
									</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeleteJobs";
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Jobs';},1500);
				}else{
					$.toast({
						heading: 'Fail',
						text: response.message,
