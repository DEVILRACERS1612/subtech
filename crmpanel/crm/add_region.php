...
$sql=$db->exeQuery("select * from mi_region where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Region Form</header>
								</div>
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50" name="region_name" value="<?php echo $row['region_name'];?>" required/>
									...
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											...
											<select class="form-control" name="head_name"> <option value="">--select--</option>
											</select>
										</div>
									</div>	
									<div class="col-md-12 col-sm-12">
									...
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											<label>Phone </label>
											<input type="text" class="form-control" maxlength="50" name="phone" value="<?php echo $row['phone'];?>" />
										</div>
									</div>	
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											<label>Fax </label>
											<input type="text" class="form-control" maxlength="50" name="fax" value="<?php echo $row['fax'];?>" />
										</div>
									</div>	
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											<label>Email </label>
											<input type="email" class="form-control" maxlength="50" name="email" value="<?php echo $row['email'];?>" />
										</div>
									</div>	
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_region where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$row['region_name']."</td><td><a href='".BASE_PATH."Add_Region/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/REGION/',
			method:'post',
			data:datastr,
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					if(response.edit=="ok")
					{
						setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Region/';},1500);
					}
				}
				else
				{
