...
$sql=$db->exeQuery("select * from mi_category where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Category Form</header>
						...
						<div class="card-body ">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select * from mi_category where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										echo "<tr><td>".$i."</td><td>".$row['cat_name']."</td><td>".$row['code']."</td><td><a href='".BASE_PATH."Add_Category/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' data-per='".$page_permission."'><i class='fa fa-trash-o '></i></a></td></tr>";
										$i++;
									}
									?>
									</tbody>
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Category/';},1500);
				}
				else
				{
					$.toast({
