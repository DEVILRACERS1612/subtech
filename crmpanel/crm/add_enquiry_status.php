...
$sql=$db->exeQuery("select * from mi_enquiry_status where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
...
<html lang="en">
<head>
<title>ADD/EDIT Enquiry Status </title>
				...
				<div class="page-content">
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Enquiry Status</header>
								</div>
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-10 col-sm-10">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50" name="enquiry_status[]" value="<?php echo $row['enquiry_status'];?>" required/>
									...
									<div class="col-md-6 col-sm-6">
										<div class="form-group">
											<label>Level</label>
											<input type="text" class="form-control" maxlength="10" name="enquiry_level[]" value="<?php echo $row['enquiry_level'];?>"/>
									...
									<div class="col-md-6 col-sm-6">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="10" name="probability[]" value="<?php echo $row['probability'];?>"/>
						...
						<div class="card-box">
							<div class="card-head">
								<header>All Enquiry Status </header>
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_enquiry_status where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$row['enquiry_status']."</td><td>".$row['enquiry_level']."</td><td>".$row['probability']."</td><td><a href='".BASE_PATH."Add_Enquiry_Status/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/ENQUIRY/',
			method:'post',
			data:datastr,
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					if(response.edit=="ok")
					{
						setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Enquiry_Status/';},1500);
					}
				}
				else
				{
	...
	$("body").on("click","#addmore",function(e){
		e.preventDefault();
		$("#moredata").append('<div class="row" style="background:#ccc;margin-bottom:2px;padding:5px 0px;"><div class="col-md-12 col-sm-12"><div class="form-group"><label>Enquiry Status Name *</label><input type="text" class="form-control" maxlength="50" name="enquiry_status[]" value="" /></div></div><div class="col-md-6 col-sm-6"><div class="form-group"><label>Level</label><input type="text" class="form-control" maxlength="10" name="enquiry_level[]" value=""/></div></div><div class="col-md-6 col-sm-6"><div class="form-group"><label>Probability (%)</label><input type="text" class="form-control" maxlength="10" name="probability[]" value=""/></div></div></div>');
	});
