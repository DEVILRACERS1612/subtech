...
$sql=$db->exeQuery("select * from mi_solcat where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
...
<html lang="en">
<head>
<title>ADD/EDIT Solution Category </title>
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Solution Category Form</header>
								...
								<input type="hidden" name="method" value="solcat" />
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-6 col-sm-6">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50" name="cat_name" value="<?php echo $row['cat_name'];?>" required/>
									...
									<div class="col-md-4 col-sm-6">
										<div class="form-group">
											...
											if($row['image']!='')
											{
												echo '<img id="upload1" src="'.WEB_PATH.'images/solcat_img/'.$row['image'].'" class="img-responsive" />';
											}else{
						...
						<div class="card-box">
						<div class="card-head">
							<header>All Solution Category </header>
						</div>
						<div class="card-body ">
							<div class="row">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select * from mi_solcat where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
									$i=1;
									while($row=$qr->fetch_assoc())
									{//<td>".$row['code']."</td>
										echo "<tr><td>".$i."</td><td>".$row['cat_name']."</td><td><a href='".BASE_PATH."Add_Solcat/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' data-per='".$page_permission."'><i class='fa fa-trash-o '></i></a></td></tr>";
										$i++;
									}
									?>
									</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeleteSolcat";
	...
	function display()
	{
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+per1+"&method=ViewSolcat";
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/CATEGORY/',
			method:'post',
			data:datastr,
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Solcat/';},1500);
				}
				else
				{
					$.toast({
