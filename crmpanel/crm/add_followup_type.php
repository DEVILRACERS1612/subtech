...
$sql=$db->exeQuery("select * from mi_followup_type where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
...
<html lang="en">
<head>
<title>ADD/EDIT Followup Type </title>
				...
				<div class="page-content">
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Followup Type</header>
								</div>
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-10 col-sm-10">
										<div class="form-group">
											<label>Followup Type Name *</label>
											<input type="text" class="form-control" maxlength="50" name="followup_type[]" value="<?php echo $row['followup_type'];?>" required/>
						...
						<div class="card-box">
							<div class="card-head">
								<header>All Followup Type </header>
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_followup_type where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$row['followup_type']."</td><td><a href='".BASE_PATH."Add_Followup_Type/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/FOLLOWUP/',
			method:'post',
			data:datastr,
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					if(response.edit=="ok")
					{
						setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Followup_Type/';},1500);
					}
				}
				else
				{
	...
	$("body").on("click","#addmore",function(e){
		e.preventDefault();
		$("#moredata").append('<div class="col-md-12 col-sm-12"><div class="form-group"><input type="text" class="form-control" maxlength="50" name="followup_type[]" value="" /></div></div>');
	});
