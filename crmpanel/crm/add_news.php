...
$sql=$db->exeQuery("select * from mi_news where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
						...
						<div class="card-body ">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select * from mi_news where  mi_status='Yes'");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										echo "<tr><td>".$i."</td><td><img src='".WEB_PATH."images/news_img/".$row['image']."' style='width:100px;' /><br>".$row['title']."</td><td><a href='".BASE_PATH."Add_News/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."'><i class='fa fa-trash-o '></i></a></td></tr>";
										$i++;
									}
									?>
									</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeleteNews";
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_News';},1500);
				}else{
					$.toast({
						heading: 'Fail',
						text: response.message,
