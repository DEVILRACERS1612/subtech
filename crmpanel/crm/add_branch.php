...
$sql=$db->exeQuery("select * from mi_branch where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Branch Form</header>
								</div>
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											...
											<select class="form-control" name="region_id"> <?php echo $objregion->region_list($row['region_id']);?>
											</select>
										</div>
									</div>
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50" name="branch_name" value="<?php echo $row['branch_name'];?>" required/>
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_branch where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$objregion->regionname($row['region_id'])."</td><td>".$row['branch_name']."</td><td>".$row['address']."</td><td>".$row['phone']."</td><td>".$row['email']."</td><td><a href='".BASE_PATH."Add_Branch/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					if(response.edit=="ok")
					{
						setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Branch/';},1500);
					}
				}
				else
				{
