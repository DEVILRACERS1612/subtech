...
include 'Model/class.subcat.php';

$page="news";
$del_id=(isset($_REQUEST['del_id']) and $_REQUEST['del_id'] !='')?$db->filterVar($_REQUEST['del_id']):'';
if(!empty($del_id))
{
	$q=$db->exeQuery("select * from nt_news where id='".$del_id."' and nt_status='Yes'");
	$r=$q->fetch_assoc();
	unlink("../../images/newsimg/".$r['image']);
	$db->exeQuery("delete from nt_news where id='".$del_id."' and nt_status='Yes'");
	$error="<span class='alert alert-danger'>Data Deleted Successfully</span>";
	header('Refresh:1;url='.BASE_PATH.'All_news/');
}
												...
												<div class="card card-box">
													<div class="card-head">
														...
														<div class="btn-group pull-right" style="padding-right:10px;padding-bottom:5px;">
															<a href="<?php echo BASE_PATH;?>Add_News/" id="addRow" class="btn btn-primary">
													...
													<div class="card-body ">
														...
														<div class="table-scrollable">
															<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
																...
																<tbody>
																<?php 
																	$qr=$db->exeQuery("select * from nt_news where nt_status='Yes' order by id desc");
																	$i=1;
																	while($row=$qr->fetch_assoc() )
																	{
																		...
																		<tr>
																		<td>'.$i.'</td>
																		<td>'.$objcat->catname($row['cat']).'</td>
																		<td>'.$objscat->subcatname($row['subcat']).'</td>
																		<td>'.$row['title'].'</td>
																		<td>'.substr($row['news'],0,110).'</td>
																		<td>';
																		if($row['image']!=''){
																			echo '<img src="'.WEB_PATH.'images/newsimg/'.$row['image'].'" style="height:50px;" />';
																		}
																		
																		
																		echo '</td>
																		<td>
																			<a href="'.BASE_PATH.'Add_News/'.$row['id'].'/" class="btn btn-primary btn-xs">
																				<i class="fa fa-pencil"></i>
																			</a>
																			<a class="btn btn-danger btn-xs" href="'.BASE_PATH.'All_News/'.$row['id'].'/" onclick="return confirm(\'Are You sure to Delete this Record \');">
