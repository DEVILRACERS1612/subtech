...
$adq=$db->exeQuery("select max(inv_no) as inv_no from mi_billing where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' ");
$ad=$adq->fetch_assoc();
if($ad['inv_no']=='')
{
	$inv_no='1';
...
$sql=$db->exeQuery("select * from mi_billing where id='".$edit_id."' and cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
if($edit_id!='')
{
	$inv_no=$row['inv_no'];
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="card-box">
								<div class="card-head">
									<header>Billing Item(s) Information</header>
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-4 col-sm-6">
										<div class="form-group">
											<label>Party *</label>
											<select class="form-control select2" name="admno" required>
												<?php echo $objparty->party_list($row['admno']);?>
											</select> 
									...
									<div class="col-md-4 col-sm-6">
										<div class="form-group">
											...
											<input type="text" class="form-control" name="inv_no"  value="<?php echo $inv_no;?>" required/>
										</div>
									</div>
									<div class="col-md-4 col-sm-6">
										<div class="form-group">
									...
									if($edit_id!='')
									{
										$qr=$db->exeQuery("Select * from mi_billing_detail where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."'  and inv_id='".$edit_id."' and mi_status='Yes'");
									...
									<div class="row" style="margin:0;">
										...
										<div class="col-md-1 col-sm-1 pl-5">
											<div class="form-group">
												<select class="form-control cat" data-id="<?php echo $sr;?>" name="cat_id[]" >
										...
										<div class="col-md-2 col-sm-2 pl-5">
											<div class="form-group">
												<select class="form-control itm" data-id="<?php echo $sr;?>" id="itm<?php echo $sr;?>" name="item_id[]" >
										...
										<div class="col-md-1 col-sm-1 pl-5">
											<div class="form-group">
												<input type="text" class="form-control" id="rate<?php echo $sr;?>" name="rate[]" value="" readonly />
											</div>
										</div>
										<div class="col-md-1 col-sm-1 pl-5">
											<div class="form-group">
												<input type="text" class="form-control drate" data-id="<?php echo $sr;?>" id="drate<?php echo $sr;?>" name="drate[]" value="" />
											</div>
										</div>
										<div class="col-md-1 col-sm-1 pl-5">
											<div class="row">
											<div class="col-md-8 col-sm-8" style="padding-right:1px">
												<div class="form-group">
													<input type="text" class="form-control qty" data-id="<?php echo $sr;?>" id="qty<?php echo $sr;?>" name="qty[]" value="" />
													<input type="hidden" id="stkqty<?php echo $sr;?>" name="" />
												</div>
											</div>
											<div class="col-md-4 col-sm-4 p-0">
												<input type="hidden" id="utid<?php echo $sr;?>" name="unit_id[]" />
												<label id="ut<?php echo $sr;?>">&nbsp;</label>
											</div>
											</div>
										...
										<div class="col-md-1 col-sm-1 pl-5">
											<div class="form-group">
												<input type="text" class="form-control total" id="total<?php echo $sr;?>" name="total[]" value=""readonly />
											</div>
										</div>
										<div class="col-md-1 col-sm-1 pl-5">
											<div class="form-group">
												<input type="text" class="form-control gst" data-id="<?php echo $sr;?>" id="gst<?php echo $sr;?>" name="gst[]" value="" />
											</div>
										</div>
										<div class="col-md-1 col-sm-1 pl-5">
											<div class="form-group">
												<input type="text" class="form-control gsttotal" id="gsttotal<?php echo $sr;?>" name="gsttotal[]" value="" readonly />
											</div>
										</div>
										<div class="col-md-1 col-sm-1 pl-5">
											<div class="form-group">
												<input type="text" class="form-control subtotal" id="subtotal<?php echo $sr;?>" name="subtotal[]" value="" readonly />
											</div>
										</div>
										<div class="col-md-1 col-sm-1 ">
											<div class="form-group">
...
$(document).ready(function(){
	...
	$("body").on("change",".itm",function(e){
		...
		$("#preloader").show();
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/BILLING/',
			type:'post',
			data:datastr,
			success:function(data){
	...
	$("body").on("keyup",".qty, .gst,.drate,#fright,#adjustment",function(e){
		...
		var stkqty=0;
		qty=$("#qty"+id).val();
		stkqty=$("#stkqty"+id).val();
		
		if(Number(qty) > Number(stkqty))
		{
			$.toast({
					heading: 'Fail',
					text: "Only "+stkqty+" quantity in Stock",
					position: 'mid-center',
					stack: false,
					icon:'error'
				});
			var qty=$("#qty"+id).val(stkqty);	
		}else{
			total(id);
		}
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>All_Billing/';},2000);
				}
				else
				{
					$.toast({
