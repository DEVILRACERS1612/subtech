...
$sql=$db->exeQuery("select * from mi_complaint_source where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
...
<html lang="en">
<head>
<title>ADD/EDIT Complaint Source </title>
							...
							<div class="card-body " id="bar-parent2">
								...
								<div class="col-md-10 col-sm-10">
									<div class="form-group">
										...
										<input type="text" class="form-control" maxlength="50" name="source[]" value="<?php echo $row['source'];?>" required />
									</div>
								</div>	
								<div class="col-md-2">
									<div class="form-group">
										<label> &nbsp; </label>
										<a href="#" id="addmore" class="btn-sm btn-primary form-control text-center" style="padding:4px 0px;font-size:11px;"><i class="fa fa-plus"></i></a>
									</div>
								</div>
								<div id="moredata" style="width:100%;"></div>
								<div id="msg"></div>
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_complaint_source where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$row['source']."</td><td><a href='".BASE_PATH."Add_CSource/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
...
$(document).ready(function(){
	...
	function display()
	{
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+per1+"&method=ViewSource";
		$("#displaydata").html("<i class='fa fa-spinner fa-spin'></i>");
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/SOURCE/',
			method:'post',
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					if(response.edit=="ok")
					{
						setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_CSource/';},1500);
					}
				}else{
					$.toast({
						heading: 'Fail',
