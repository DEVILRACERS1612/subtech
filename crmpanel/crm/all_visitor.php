...
$db->exeQuery("update mi_notifications_soft set read_status='1' where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and noti_for='".$_SESSION[SITE_NAME]['MICMP_userid']."' and noti_page_for='visitor' and read_status='0'");
?>
<!DOCTYPE html>
<html lang="en">
<head>
						...
						<div class="card-body ">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select * from mi_visitor where mi_status='Yes' order by rdate desc");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										echo "<tr><td>".$i.".</td><td>".date("d M,Y H:i",strtotime($row['rdate']))."</td><td>".$row['cname']."</td><td>".$row['mobile']."</td><td>".$row['subject']."</td><td>".$row['message']."</td></tr>";
										$i++;
									}
									?>
									</tbody>
