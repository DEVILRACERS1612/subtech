...
$sql=$db->exeQuery("select * from mi_subcategory where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
...
<html lang="en">
<head>
<title>ADD/EDIT Product Sub-Category </title>
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Product Sub-Category Form</header>
								...
								<input type="hidden" name="method" value="Subcat" />
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-6 col-sm-6">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50" name="subcat_name" value="<?= $row['subcat_name']?>" required/>
										</div>
									</div>
									<div class="col-md-6 col-sm-6">
										<div class="form-group">
						...
						<div class="card-box">
						<div class="card-head">
							<header>All Product Sub Category </header>
						</div>
						<div class="card-body ">
							<div class="row">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select * from mi_subcategory where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										echo "<tr><td>".$i."</td><td>".$row['subcat_name']."</td><td>".$row['code']."</td><td><a href='".BASE_PATH."Add_SubCategory/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."'><i class='fa fa-trash-o '></i></a></td></tr>";
										$i++;
									}
									?>
									</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeleteSubcat";
	...
	function display()
	{
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+per1+"&method=Viewsubcat";
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/CATEGORY/',
			method:'post',
			data:datastr,
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_SubCategory/';},1500);
				}
				else
				{
					$.toast({
