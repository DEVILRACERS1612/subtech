						...
						<div class="card-body ">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select a.*,b.title,b.urlname from mi_comment a left join mi_blogs b on b.id=a.blog_id where a.mi_status='Yes' order by a.rdate desc");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										echo "<tr><td>".$i."</td><td>".date("d M,Y H:i",strtotime($row['rdate']))."</td><td><a href='".WEB_PATH."blog/".$row['urlname']."' target='_blank' >".$row['title']."</a></td><td>".$row['cname']."</td><td>".$row['email']."</td><td>".$row['comment']."</td><td>".$row['act_status']."</td><td>";
										if($row['act_status']=='Pending'){
											echo "<a href='#' title='Verify' class='btn btn-xs btn-primary verify' data-id='".$row['id']."'><i class='fa fa-check'></i></a>";
											echo "<a href='#' title='Delete' class='btn btn-xs btn-danger delme' data-id='".$row['id']."'><i class='fa fa-trash'></i></a>";
										}
...
$(document).ready(function(){
	...
	$("body").on("click",".verify",function(e){
		...
		var cnf=confirm("Are you want to verify this comment");
		e.preventDefault();
		if(cnf==false){return false;}
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=verifycomment";
	...
	$("body").on("click",".delme",function(e){
		var did=$(this).attr("data-id");
		//var pgpmsn=$(this).attr("data-per");
		var cnf=confirm("Are you want to Delete this Comment");
		e.preventDefault();
		if(cnf==false){return false;}
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=deletecomment";
