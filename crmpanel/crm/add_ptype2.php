...
$sql=$db->exeQuery("select * from mi_ptype2 where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
...
<html lang="en">
<head>
<title>ADD/EDIT Product Type2 </title>
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Product Type2</header>
								...
								<input type="hidden" name="method" value="Ptype2" />
						...
						<div class="card-box">
						<div class="card-head">
							<header>All Product Type 2</header>
						</div>
						<div class="card-body ">
							<div class="row">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select * from mi_ptype2 where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										echo "<tr><td>".$i."</td><td>".$row['cat_name']."</td><td>".$row['code']."</td><td><a href='".BASE_PATH."Add_Ptype2/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' data-per='".$page_permission."'><i class='fa fa-trash-o '></i></a></td></tr>";
										$i++;
									}
									?>
									</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeletePtype2";
	...
	function display()
	{
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+per1+"&method=Viewptype2";
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/CATEGORY/',
			method:'post',
			data:datastr,
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Ptype2/';},1500);
				}
				else
				{
					$.toast({
