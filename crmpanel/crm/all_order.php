...
//$row=$sql->fetch_assoc();
$db->exeQuery("update mi_notifications_soft set read_status='1' where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and noti_for='".$_SESSION[SITE_NAME]['MICMP_userid']."' and noti_page_for='order' and read_status='0'");
?>
<!DOCTYPE html>
<html lang="en">
<head>
<title>All Product Order </title>
			...
			<div class="page-content-wrapper">
				<div class="page-content">
					...
					<div class="row">
						...
						<div class="card-box">
						<div class="card-head">
							<header>All Product Order </header>
						</div>
						<div class="card-body ">
							<div class="row">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									$qr=$db->exeQuery("select * from mi_order where mi_status='Yes' order by rdate desc");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										//<td>".$row['email']."</td>
										echo "<tr><td>".$i."</td><td>".date("d M,Y H:i",strtotime($row['rdate']))."</td><td>".$row['cname']."</td><td>".$row['mobile']."</td><td>".$row['pname']."</td><td>".$row['qty']."</td><td>".$row['message']."</td></tr>";
										$i++;
									}
									?>
									</tbody>
