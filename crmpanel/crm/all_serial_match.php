			...
			<div class="table-scrollable">
				<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
					...
					FROM
						mi_product_detail
					WHERE
						cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status = 'Yes'
					GROUP BY
						prd_id
					ORDER BY
						prd_id;");
				...
				while($row=$qr->fetch_assoc())
				{
					...
					echo "<tr><td>".$i."</td><td>".$pd['pname']."</td><td>".$pd['model']."</td><td>".$objcat->brand_name($row['brand'])."</td><td>".$row['serial_count']."</td><td>".$row['cust_id_count']."</td><td>".$img."</td><td><a href='#' data-id='".$row['prd_id'],"' class='btn btn-primary btn-xs viewall' title='View'><i class='fa fa-eye'></i></a></td></tr>";
					$i++;
				}
					?>
					</tbody>
