										...
										<div class="row">
											<div class="col-md-12">
												<div class="card card-box">
													<div class="card-head">
														<header>All Suppliers</header>
														<div class="btn-group pull-right" style="padding-right:10px; padding-bottom:5px;">
															<a href="<?php echo BASE_PATH;?>Add_Party/" id="addRow" class="btn btn-primary">
																Add New Suppliers <i class="fa fa-plus"></i>
															</a>
														</div>
													</div>
													<div class="card-body ">
														...
														<div class="table-scrollable">
															<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
																...
																<?php
																//echo "select * from mi_user where school_id='".$_SESSION['MICMP_schoolid']."' and mi_status='Yes'";
																$qr=$db->exeQuery("select * from mi_party where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
																$i=1;
															while($row=$qr->fetch_assoc())
															{
																...
																echo "<tr><td>".$i."</td><td>".$row['party_name']."</td><td>".$row['gstin']."</td><td>".$row['email']."</td><td>".$row['mobile']."</td><td>".$row['address']."</td><td>".$row['other_detail']."</td><td>".$img."</td><td><a href='".BASE_PATH."Add_Party/Edit/".$row['id']."/' class='btn btn-primary btn-xs' title='Edit'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."'  title='Delete'><i class='fa fa-trash-o'></i></a></td></tr>";
																$i++;
															}
																?>
																</tbody>
