...
$sql=$db->exeQuery("select * from mi_blogs where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
		...
		<div class="page-container">
					...
					<div class="row">
						<div class="col-sm-7">
							<div class="card-box">
								...
								<input type="hidden" name="method" value="Blogs" />
			...
			<div class="col-md-12 col-sm-6">
				<div class="row">
					...
					<div class="col-md-3">
						<div class="form-group">
							<label> &nbsp; </label>
						<a href="<?=BASE_PATH?>Add_Bcat" class="btn btn-sm btn-primary d-block" >Add Category</a>
						</div>
					</div>
			...
			<div class="col-md-12 col-sm-6">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="url" name="urlname" value="<?php echo $row['urlname'];?>" placeholder="URL" required />
				</div>
			</div>
			<div class="col-md-12 col-sm-6">
				<div class="form-group">
				<label>Short Content</label>
				<textarea name="sdes" class="form-control" cols="30" rows="5"><?php echo $row['sdes'];?></textarea>
				</div>
			</div>
			<div class="col-md-12 col-sm-6">
				<div class="form-group">
				<label>Blog Content</label>
				<textarea name="blogs" id="summernote" cols="30" rows="10"><?php echo $row['blogs'];?></textarea>
				</div>
			</div>
			<div class="col-md-12 col-sm-6">
				<div class="form-group">
					<label>Author *</label>
					<input type="text" class="form-control" id="" name="author" value="<?php echo $row['author'];?>" required />
			...
			<div class="col-md-9 col-sm-6">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="" name="alttext" value="<?php echo $row['alttext'];?>" placeholder="Alt/Title Text" />
				</div>
			</div>
			<div class="col-md-2 col-sm-6">
				<div class="form-group">
					...
					if($row['image']!='')
					{
						echo '<img id="upload1" src="'.WEB_PATH.'images/blog_img/'.$row['image'].'" class="img-responsive" />';
					}else{
			...
			<div class="col-md-12 col-sm-6">
			 <div class="form-group">
				...
				<textarea  name="mtitle" class="form-control"><?php echo $row['mtitle'];?></textarea>
			</div>
			</div>
			<div class="col-md-12 col-sm-6">
			 <div class="form-group">
				<label>Meta Description</label>
				<textarea  name="mdes" class="form-control"><?php echo $row['mdes'];?></textarea>
			</div>
			</div>
			<div class="col-md-12 col-sm-6">
			 <div class="form-group">
				<label>Meta Keywords</label>
				<textarea  name="mkeywords" class="form-control"><?php echo $row['mkeywords'];?></textarea>
			</div>
			</div>
			<div class="col-md-12 col-sm-6">
			 <div class="form-group">
				<label>Tags (Use Enter Key for more tag)</label>
				<input type="text" name="mtags" class="form-control tags tags-input" data-type="tags" value="<?php echo $row['mtags'];?>" />
			</div>
			</div>
			<div class="col-md-12 col-sm-6">
			 <div class="form-group">
				<label>Schema Text</label>
				<textarea  name="schematext" class="form-control"><?php echo $row['schematext'];?></textarea>
			</div>
			</div>
						...
						<div class="card-body ">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select * from mi_blogs where  mi_status='Yes'");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										$crow=$db->exeQuery("select count(id) as cnt from mi_comment where blog_id='".$row['id']."' and mi_status='Yes'")->fetch_assoc();
										echo "<tr><td>".$i."</td><td>".$row['title']."</td><td>".$crow['cnt']."</td><td><a href='".BASE_PATH."Add_Blog/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."'><i class='fa fa-trash-o '></i></a></td></tr>";
										$i++;
									}
									?>
									</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeleteBlog";
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Blog';},1500);
				}
				else
				{
					$.toast({
