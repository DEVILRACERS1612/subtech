...
//$row=$sql->fetch_assoc();
$db->exeQuery("update mi_notifications_soft set read_status='1' where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and noti_for='".$_SESSION[SITE_NAME]['MICMP_userid']."' and noti_page_for='career' and read_status='0'");
?>
<!DOCTYPE html>
<html lang="en">
<head>
<title>All Career </title>
						...
						<div class="card-body ">
							...
							<div class="table-scrollable">
								<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
									...
									<?php 
									$qr=$db->exeQuery("select a.*,j.title from mi_career a left join mi_jobs j on j.id=a.job  where a.mi_status='Yes' order by a.rdate desc");
									$i=1;
									while($row=$qr->fetch_assoc())
									{
										if(!empty($row['resume'])){
										    $dnd="<a href='".WEB_PATH."crmpanel/images/career_img/".$row['resume']."' download='".$row['mobile'].".resume.pdf'>download</a>";
										}else{
										    $dnd="";
										}
										
										echo "<tr><td>".$i."</td><td>".date("d M,Y H:i",strtotime($row['rdate']))."</td><td>".$row['cname']."</td><td>".$row['email']."</td><td>".$row['mobile']."</td><td>".$row['title']."</td><td>".$row['message']."</td><td>".$dnd."</td></tr>";
										$i++;
									}
									?>
									</tbody>
