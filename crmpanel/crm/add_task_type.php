...
$sql=$db->exeQuery("select * from mi_task_type where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
				...
				<div class="page-content">
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit Task Type</header>
								</div>
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-10 col-sm-10">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50" name="task_type[]" value="<?php echo $row['task_type'];?>" required/>
									...
									<div class="col-md-2">
										<div class="form-group">
											<label> &nbsp; </label>
											<a href="#" id="addmore" class="btn-sm btn-primary form-control text-center" style=""><i class="fa fa-plus fa-2x"></i></a>
										</div>
									</div>
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_task_type where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$row['task_type']."</td><td><a href='".BASE_PATH."Add_Task_Type/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/TASKTYPE/',
			method:'post',
			data:datastr,
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					if(response.edit=="ok")
					{
						setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Task_Type/';},1500);
					}
				}
				else
				{
	...
	$("body").on("click","#addmore",function(e){
		e.preventDefault();
		$("#moredata").append('<div class="col-md-12 col-sm-12"><div class="form-group"><input type="text" class="form-control" maxlength="50" name="task_type[]" value="" /></div></div>');
	});
