...
if($method=='Add')
{
	...
	$sql=$db->exeQuery("select * from mi_location where id='".$edit_id."' and mi_status='Yes'");
	$row=$sql->fetch_assoc();
	$sql1=$db->exeQuery("select * from mi_state where id='".$row['state']."' and mi_status='Yes'");
...
<html lang="en">
<head>
<title>ADD/EDIT State-Location </title>
				...
				<div class="page-content">
					<div class="row">
						<div class="col-sm-5">
							<div class="card-box">
								<div class="card-head">
									<header>Add/Edit State-Location Form</header>
								</div>
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50"  value="<?php echo $state;?>" readonly/>
									...
									<div class="col-md-10 col-sm-10">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50" name="location[]" value="<?php echo $row['location'];?>" required/>
						...
						<div class="card-box">
							<div class="card-head">
								<header>All Location for <?php echo $state;?> State </header>
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_location where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and state='".$state_id."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$row['location']."</td><td><a href='".BASE_PATH."Add_Location/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeleteLocation";
	...
	function display()
	{
		...
		var datastr="state=<?php echo $state_id;?>&post_id=<?php echo $post_id;?>&pg_pmsn="+per1+"&method=ViewLocation";
		$("#displaydata").html("<i class='fa fa-spinner fa-spin'></i>");
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/COUNTRY/',
			method:'post',
	...
	$("body").on("click","#addmore",function(e){
		e.preventDefault();
		$("#moredata").append('<div class="col-md-12 col-sm-12"><div class="form-group"><input type="text" class="form-control" maxlength="50" name="location[]" value="" /></div></div>');
	});
