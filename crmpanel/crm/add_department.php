...
$sql=$db->exeQuery("select * from mi_department where id='".$edit_id."' and mi_status='Yes'");
$row=$sql->fetch_assoc();
								...
								<div class="card-body " id="bar-parent2">
									...
									<div class="col-md-12 col-sm-12">
										<div class="form-group">
											...
											<input type="text" class="form-control" maxlength="50" name="department_name" value="<?php echo $row['department_name'];?>" required />
							...
							<div class="card-body ">
								...
								<div class="table-scrollable">
									<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
										...
										<?php 
										$qr=$db->exeQuery("select * from mi_department where cmp_id='".$_SESSION[SITE_NAME]['MICMP_cmpid']."' and mi_status='Yes'");
										$i=1;
										while($row=$qr->fetch_assoc())
										{
											echo "<tr><td>".$i."</td><td>".$row['department_name']."</td><td><a href='".BASE_PATH."Add_Department/Edit/".$row['id']."/' class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></a><a class='btn btn-danger btn-xs delme' data-id='".$row['id']."' ><i class='fa fa-trash-o '></i></a></td></tr>";
											$i++;
										}
										?>
										</tbody>
...
$(document).ready(function(){
	...
	$("body").on("click",".delme",function(e){
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+pgpmsn+"&del_id="+did+"&method=DeleteDepartment";
	...
	function display()
	{
		...
		var datastr="post_id=<?php echo $post_id;?>&pg_pmsn="+per1+"&method=ViewDepartment";
		$("#displaydata").html("<i class='fa fa-spinner fa-spin'></i>");
		$.ajax({
			url:'<?php echo BASE_PATH;?>Controller/DESIGNATION/',
			method:'post',
		...
		$.ajax({
			...
			success:function(data){
				...
				if(response.type=="success")
				{
					...
					if(response.edit=="ok")
					{
						setTimeout(function(){window.location='<?php echo BASE_PATH;?>Add_Department/';},1500);
					}
				}
				else
				{
