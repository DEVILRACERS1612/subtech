...
if(!empty($del_id))
{
	$q=$db->exeQuery("select * from am_products where id='".$del_id."' and am_status='Yes'");
	$r=$q->fetch_assoc();
	$picture='./images/product/'.$r['image'];
	chmod($picture, 0644);
	unlink($picture);
	$db->exeQuery("delete from am_products where id='".$del_id."' and am_status='Yes'");
	$error="<span class='alert alert-danger'>Data Deleted Successfully</span>";
	header('Refresh:5;url='.BASE_PATH.'All_product/');
}
												...
												<div class="card card-box">
													<div class="card-head">
														...
														<div class="btn-group pull-right" style="padding-right:10px; padding-bottom:5px;">
															<a href="<?php echo BASE_PATH;?>Add_product/" id="addRow" class="btn btn-primary">
													...
													<div class="card-body ">
														...
														<div class="table-scrollable">
															<table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="exportTable">
																...
																<tbody>
																<?php 
																	$qr=$db->exeQuery("select * from am_products where am_status='Yes'");
																	$i=1;
																	while($row=$qr->fetch_assoc() )
																	{
																		...
																		<tr>
																		<td>'.$i.'</td>
																		<td>'.$objcat->catname($row['cat_id']).'</td>
																		<td>'.$row['pname'].'</td>
																		<td>'.$objcolor->colorname($row['color']).'</td>
																		<td>'.$objuom->uomname($row['uom']).'</td>
																		<td>'.$row['price'].'</td>
																		<td>';
																		if($row['image']!=''){
																			echo '<img src="'.BASE_PATH.'images/product/'.$row['image'].'" style="height:50px;" />';
																		}
																		...
																		<td>
																			<a href="'.BASE_PATH.'Add_bom/'.$row['id'].'/" class="btn btn-primary btn-xs" title="Add Manufacturing Formula"><i class="fa fa-plus"></i>
																			</a>
																			<a href="'.BASE_PATH.'Add_product/'.$row['id'].'/" class="btn btn-primary btn-xs" title="Edit">
																				<i class="fa fa-pencil"></i>
																			</a>
																			<a class="btn btn-danger btn-xs" href="'.BASE_PATH.'All_product/'.$row['id'].'/" onclick="return confirm(\'Are You sure to Delete this Record \');" title="Delete">
